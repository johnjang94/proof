...
type FilterTab = "ALL" | "Project" | "Discussions" | "Interview Status";
...
export default function Notice() {
  ...
  const fetchAndMarkRead = useCallback(async () => {
    try {
      ...
      setNewNotifs(sorted.filter((n) => !n.isRead));
      setArchiveNotifs(sorted.filter((n) => n.isRead));

      const unreadIds = sorted.filter((n) => !n.isRead).map((n) => n.id);
                  ...
                  {filterNotifs(newNotifs).map((n) => (
                    <NotifRow
                      ...
                      onViewDetail={() =>
                        n.projectId &&
                        router.push(
                          `/notification/client/shortlist/${n.projectId}`,
                        )
                      }
                    />
                  ))}
