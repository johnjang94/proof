...
export default function Main() {
    ...
    const loadAppliedProjects = async () => {
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error(
            `Failed to load applied projects (${res.status}). ${text}`,
          );
        }
        ...
        setAppliedError(
          err instanceof Error
            ? err.message
            : "Failed to load applied projects.",
        );
      } finally {
  ...
  const visibleAppliedProjects = useMemo(
    () => sortByCreatedAtDesc(appliedProjects).slice(0, 3),
    [appliedProjects],
  );
              ...
              return (
                ...
                >
                  <div className="relative mb-3 aspect-video w-full overflow-hidden rounded-[22px] bg-neutral-200">
                    ...
                    <div className="absolute left-3 top-3 rounded-full bg-black/65 px-2.5 py-1 text-[10px] font-medium uppercase tracking-[0.08em] text-white">
                      Applied
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3 px-1">
                    ...
                    <div className="min-w-0 flex-1">
                      <h3 className="line-clamp-2 text-[1.03rem] font-medium leading-tight tracking-[-0.01em] text-neutral-900">
                        {project.projectName}
                      </h3>
